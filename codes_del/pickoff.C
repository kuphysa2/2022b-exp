...
void pickoff()
{
    ...
    // recording y(t)
    char ofs_yt_name[64];
    snprintf(ofs_yt_name, 64, "../exp%04d/a%04d/cut/yt%d.dat", exp_date, ana_date, adc_channel + 1);
    ...
    canvases[0]->Update();
    canvases[0]->Print(Form("../exp%04d/a%04d/cut/PickOff4yDistrib%d.pdf", exp_date, ana_date, adc_channel + 1));
    canvases[1]->Update();
    canvases[1]->Print(Form("../exp%04d/a%04d/cut/PickOff4SDistrib%d.pdf", exp_date, ana_date, adc_channel + 1));
    ...
    graph_f->Draw("AP");
    canvases[2]->Update();
    canvases[2]->Print(Form("../exp%04d/a%04d/cut/ft%d.pdf", exp_date, ana_date, adc_channel + 1));
    for (j = 0; j < 3; j++)
    {
    ...
    histogramTDC->Draw();
    canvases[3]->Update();
    canvases[3]->Print(Form("../exp%04d/a%04d/cut/tdcPick%d.pdf", exp_date, ana_date, adc_channel + 1));
